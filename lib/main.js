...
export default {
  ...
  warnNotif(fr, tt, options) {
    ...
    const fromTypeName = path.basename(fr).toString();
    const notif = atom.notifications;

    isSame_isBothTs ? notif.addError('ERROR: Same file path.')
      : isSame_isBothTs === false ? notif.addWarning('WARNING: Non typescript files. Same file path.')
        : isFolder ? notif.addError(`ERROR: File not found.`)
          : isDefault ? notif.addWarning(`WARNING: Unable to import ${fromTypeName} to typescript and vice versa.`)
            : 0;
  },
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors((editor) => {
      ...
      leftDock.ondragend = (event) => {
        this.subscriptions.add(lines, "drop", () => {
          leftDock.ondragend = (event) => this.calculatePath(editor);
        })
      };
